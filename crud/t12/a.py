...
def get_nested_value(data, key_path):
    """
    根据点分隔的路径（如 'layout.base_layout'）获取嵌套字典中的值。
...
def main():
    ...
    middle_data = yaml.safe_load(middle_yaml)
    child_data = yaml.safe_load(child_yaml)
    base_nested_data = yaml.safe_load(base_nested_yaml)

    # 合并所有基础数据（包括 middle_data）
    all_base_data = {**base_data, **middle_data, **base_nested_data}
    print(all_base_data)
