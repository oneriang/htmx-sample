...
@app.get("/table1/{table_name}")
async def read_table1(request: Request, table_name: str):
...
@app.get("/table2/{table_name}")
async def read_table2(request: Request, table_name: str, page: int = 1, search: str = '', page_size: int = 10):
    if table_name not in metadata.tables:
        raise HTTPException(status_code=404, detail="Table not found")
    ...
    with SessionLocal() as session:
        total_items = session.execute(query).count()
...
@app.get("/table3/{table_name}")
async def read_table3(request: Request, table_name: str, page: int = 1, search: str = '', page_size: int = 10):
    if table_name not in metadata.tables:
        raise HTTPException(status_code=404, detail="Table not found")
    ...
    with SessionLocal() as session:
        total_items = session.execute(select([table.c[primary_key]]).where(query._whereclause)).count()
        result = session.execute(query.offset(offset).limit(page_size)).fetchall()
        columns = table.columns.keys()
...
@app.get("/table4/{table_name}")
async def read_table4(request: Request, table_name: str, page: int = 1, search: str = '', page_size: int = 10):
    if table_name not in metadata.tables:
        raise HTTPException(status_code=404, detail="Table not found")
    ...
    with SessionLocal() as session:
        total_items = session.execute(select([func.count()]).select_from(query.alias())).scalar()
        result = session.execute(query.offset(offset).limit(page_size)).fetchall()
        columns = table.columns.keys()
...
@app.get("/table5/{table_name}")
async def read_table5(request: Request, table_name: str, page: int = 1, search: str = '', page_size: int = 10):
    if table_name not in metadata.tables:
        raise HTTPException(status_code=404, detail="Table not found")
...
@app.get("/table6/{table_name}")
async def read_table6(request: Request, table_name: str, page: int = 1, search: str = '', page_size: int = 10):
    if table_name not in metadata.tables:
        raise HTTPException(status_code=404, detail="Table not found")
    ...
    query = select([table])  # 将 select([table]) 改为 select(table)
...
@app.get("/table7/{table_name}")
async def read_table7(request: Request, table_name: str, page: int = 1, search: str = '', page_size: int = 10):
    if table_name not in metadata.tables:
        raise HTTPException(status_code=404, detail="Table not found")
...
@app.post("/create1/{table_name}")
async def create_item1(table_name: str, data: Dict[str, Any] = Form(...)):
    if table_name not in metadata.tables:
        raise HTTPException(status_code=404, detail="Table not found")
...
@app.post("/edit1/{table_name}/{id}")
async def edit_item1(table_name: str, id: str, data: Dict[str, Any] = Form(...)):
    if table_name not in metadata.tables:
        raise HTTPException(status_code=404, detail="Table not found")
