...
def load_data_from_yaml(filename: str = "chinook.yaml") -> Dict[str, Any]:
    try:
        ...
        logger.error(f"加载 YAML 数据时出错: {e}")
        return {}
...
def execute_all_transactions(db: SessionLocal = Depends(get_db)):
    ...
    except yaml.YAMLError as e:  # Changed from json.JSONDecodeError to yaml.YAMLError
        db.rollback()
        logger.error(f"解析 YAML 数据时出错: {e}")
        raise HTTPException(status_code=500, detail="YAML 解析错误")
    except Exception as e:
        db.rollback()
